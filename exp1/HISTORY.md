...
This records the earlier `pte_classifier` experiments. Their source package was retired when the active model was consolidated into `TimeCog_net/model.py`; commands referring to `pte_classifier` are historical. The saved results remain under `exp1/runs/`.
